...
- Full-take acquisition has no in-game authoring controls. Frame and input sources run only for the requested session. A short settling phase discards the queue click and switch residue, then starts the configured-duration clock automatically. There is no separate focus, input-test, stage-selection, or arming gate. Sessions append under stable per-game experiment folders. `session_metadata.json` stores the operator-selected role without rewriting the source manifest, and deletion moves the complete session directory to recoverable trash.
...
- `GameProfiler` extends the source-controlled `config/games` definition; it does not create a parallel profile system. The definitions contain capture/input adapter defaults and coarse control summaries. Profiling adds an evidence-backed control specification, measured behaviors, map-viewer procedure, limitations, and human edits under mutable artifact/profile storage.
...
- Semi-automatic profiling uses separate bounded sessions for ordinary cruise, rotation-only, movement-only, and straight-forward/no-turn motion. The selected capture plan supplies a machine-readable segment label and semantics; post-take review only confirms that the requested motion is visible. It estimates movement speed and direction, turning/camera response, acceleration and stopping, camera-character coupling, and input-to-observed-response latency where observable. Additional jump/dash, cooldown/recovery, collision, or other probes are requested only when the existing evidence cannot support a needed field.
...
- Calibration consumes the role-labeled rotation-only and movement-only sessions directly rather than requiring human-entered time ranges. Their full frames and any recorded inputs remain synchronized.
- A role-labeled straight-forward/no-turn session preserves a notable mini-map translation while movement direction is constrained to cursor heading. This establishes the sign and angular relationship between estimated cursor pose and mini-map shift; measured inputs are useful provenance but are optional for the visual relationship.
