...
DEFAULT_CONFIG = {
    ...
    "cursor": {
        ...
        # Bilinear sub-pixel extraction softens binary component edges.  Keep
        # the optional shape persistence gate below that interpolation band;
        # center geometry does not use this value.
        "shape_persistence_threshold": 0.65,
    },
}
...
def _cursor_components(
    ...
    """Extract optional cursor-shape components after geometry is known.

    This legacy color model is deliberately not used to find the rotation
    center.  A color mismatch may make shape unavailable, but it must not
    discard independently observable center geometry.
    """
    ...
    if not masks:
        ...
        raise RuntimeError(
            "Cursor shape component gate accepted 0/{} frames. Required HSV {}..{}, "
            ...
            "This gate applies only to optional cursor shape; the rotation "
            "center is calibrated independently from color-agnostic temporal "
            "geometry.".format(
...
def _rotation_center(movement_frames: np.ndarray, boundary: dict, config: dict) -> dict:
    ...
    return _cursor_temporal_center(
        movement_frames, initial, float(config["search_radius_px"])
    )
...
def _cursor_orbit_aligned_frames(
    frames: np.ndarray, center: dict, config: dict
) -> Tuple[np.ndarray, Sequence[float]]:
    """Render optional color-derived shape masks in one screen direction."""

    pivot = np.asarray(
        [center["metrics"]["x"], center["metrics"]["y"]], dtype=np.float64
    )
    masks, centroids, _indices = _cursor_components(frames, pivot, config)
...
def calibrate_cursor_orbit_frames(
    ...
    *,
    ...
    shape_failure = None
    try:
        aligned_frames, alignment_angles = _cursor_orbit_aligned_frames(
            frames, center, merged["cursor"]
        )
        if progress:
    ...
    result = {
        ...
        "capabilities": {
            "rotation_center": {
                ...
                "analyzed_frames": int(pivot["analyzed_frames"]),
...
def calibrate_minimap_frames(
    ...
    from rig_runtime.services.calibration.cursor.pose import estimate_cursor_pose_frames
    ...
    if ordinary_frames is not None:
        ...
        from rig_runtime.services.calibration.cursor.dynamics import summarize_cursor_dynamics
