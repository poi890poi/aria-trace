...
- Semi-automatic profiling runs controlled probes and estimates movement speed and direction, turning/camera response, acceleration and stopping, camera-character coupling, jump/dash timing, cooldown/recovery, collision response, and input-to-observed-response latency where observable. This list is extensible and does not require every game to expose every behavior.
...
- The proposed calibration recording applies continuous horizontal camera rotation in one direction while vertical input repeats `up -> down -> down -> up -> up -> down -> down -> ...`. Frames and generated-control timing are recorded together.
