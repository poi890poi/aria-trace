...
The local Acquisition Workbench is the primary PC MVP entry point. Its simplified recorder lists visible Windows windows and input adapters, then appends any number of bounded sessions. Completed sessions remain visible in one manager with a controlled label dropdown and recoverable delete action. A custom/unprofiled game remains a first-class choice.

Acquisition is zero-interruption. Selecting **Start recording** starts the chosen frame/input sources, discards queue/switch residue, and begins the bounded session after a three-second **SWITCH TO GAME** timer. There is no gameplay-focus gate, input test, hotkey, stage selection, arming step, take target, or completion input. A non-empty post-capture label is the explicit review action and promotes successful take boundaries to usable capture or route markers. Deleted sessions move under `sessions/workbench/.trash/`.
...
Start with `python -m aria_trace.apps.workbench` and open http://127.0.0.1:8765/. Arbitrary-game tests use a fake Popular Game A profile to prove that the workflow contains no Combat Master coupling and no in-game recorder commands.
...
- Current checkpoint: 45/45 tests pass. The real verifier injected F23 during the 1.5-second settling interval and F24 afterward. All four Raw Input packets were received and accepted with zero foreground rejection; only the two eligible F24 events were persisted, the first at session time zero, with ten frames and a complete manifest under `artifacts/workbench/input_verification/20260824T154301Z`. The existing confirmed gameplay capture was recovered as ready from `genshin-control-profile-20260824T153344Z` with 1,802 frames, 60.17 seconds, and ten controls.
...
Genshin Impact PC is now the first POC game profile. The integrated workbench exposes short, self-labeled ordinary-cruise, rotation-only, movement-only, and straight-forward/no-turn stages before full-map viewer capture and repeated route recording. The new forward segment requests a long straight run with no camera turn and notable mini-map translation, preserving the cursor-heading-to-map-shift relationship for pose refinement. Short stages use a settled timer start and declare input optional, so they work with synchronized controls or as visual-only captures; full-map and route stages retain first-input start and required input health. Profile drafts remain under workbench artifacts. The versioned evidence index now exposes segment labels, semantics, start trigger, and input requirement alongside source sessions, markers, timing/count summaries, drops, and profile provenance.

The canonical recorder preserves full-frame video, exact frame timing, raw PC or Android input evidence, source configuration, drop counts, and append-only markers. Source-controlled `config/games` definitions record capture/input defaults and coarse control summaries, while route definitions remain under `config/routes`. Mutable phone-game and rig profiles are separate under `profiles/`. The current `ReplayPackage` compiler samples reference frames, descriptors, route stages, and action priors with links to source-frame indices.

Circular mini-map calibration is now implemented for profiled games. The workbench selects complete confirmed rotation-only and movement-only sessions by their capture-plan labels, eliminating four manual time-boundary fields, and records the forward/no-turn session in calibration provenance as the heading-motion reference. The estimator fits the outer boundary from robust radial transition points, estimates the independent cursor pivot from the movement centroid orbit, learns the screen-fixed cursor template from camera-rotation persistence, and stores calibration.json, model.npz, confidence components, and review PNGs. Cursor pose uses the fitted cursor pivot as the polar origin and fits a circular Gaussian to each angular-correlation lobe; per-frame mean, width, center standard error, fit R², peak separation, IoU, and centroid agreement are retained. Existing legacy single-session calibration remains readable.
...
- quality-bearing XY shift estimation and a semantic transform from raw cursor screen angle to world/game heading;
...
1. **Labeled gameplay segments and game profiler.** Record short ordinary-cruise, rotation-only, movement-only, and straight-forward/no-turn sessions with ordinary gameplay UI visible. Extend the existing game profile from this input/observation evidence and human editing. Record semantic actions, physical bindings, activation semantics, compatible combinations, movement/camera response, relevant timing, confidence, provenance, and unknowns. The capture plan is the label; the reviewer only confirms that the requested motion is visible.
