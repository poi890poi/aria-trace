...
- **CAL-03C:** Measure camera-view/phone-screen coverage, camera utilization, screen-view IoU, task-ROI coverage, and geometry uncertainty. Measure image-chain resolution with the ISO 12233:2024 slanted-edge e-SFR procedure and retain the full curve plus derived MTF50/MTF10. Keep the two sampled grids explicit: report the primary display-referred result in cycles per display pixel, retain cycles per camera pixel as the native analysis/audit axis, and report physical cycles per millimetre only when display pitch is measured. Treat one-display-pixel dark/bright lines as the `0.5 cycles/display-pixel` Nyquist endpoint, not as an unqualified `1 line/pixel` resolution claim. Evaluate ADB-to-camera and camera-to-camera feature matching separately with ground-truth repeatability, matching score, MMA at declared reprojection thresholds, match counts, spatial coverage, and downstream geometry/pose error. Do not introduce a project-defined scalar as if it were a standard metric.
