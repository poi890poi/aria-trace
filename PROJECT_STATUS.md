...
Acquisition is now zero-interruption. Queueing a take starts frame and input sources for that bounded capture lifetime. A 1.5-second **SWITCH TO GAME** settling interval discards the queue click and switch residue; after **PLAY TO START**, the first qualifying input is retained at session time zero and starts the configured-duration clock. There is no gameplay-focus gate, input test, hotkey, or stage/completion input. The armed experiment is persisted atomically and restored after restart; installations without prior state recover the newest valid session context. Successful captures receive take_start and take_end evidence boundaries; route_start and route_complete are created only through explicit post-take confirmation.
