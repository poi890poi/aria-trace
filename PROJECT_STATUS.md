...
Acquisition is now zero-interruption. Queueing a take starts frame and input sources for that bounded capture lifetime, while pre-start observations are discarded. The first qualifying input received is retained at session time zero and starts the configured-duration clock. There is no gameplay-focus gate, input test, hotkey, or stage/completion input. Successful captures receive take_start and take_end evidence boundaries; route_start and route_complete are created only through explicit post-take confirmation. Compilation remains blocked until each take is confirmed.
...
- Current full-suite checkpoint: 43/43 passing after removal of the erroneous focus gate. A real Windows verifier against the visible Genshin window injected F24, received and accepted both make/break packets with zero rejection, persisted two inputs and ten frames, set the first input to session time zero, and closed a complete manifest under `artifacts/workbench/input_verification/20260824T150552Z`.
