...
5. Select **Queue next capture** and switch to Genshin while the upper-right HUD says **ARMING** or **SWITCH TO GAME**. Queue/switch input is discarded for 1.5 seconds. When the HUD says **PLAY TO START**, the next active input starts the session and the `REC · mm:ss` countdown.
...
3. Switch to the selected game window during **ARMING** / **SWITCH TO GAME**. After the 1.5-second settling interval, the HUD changes to **PLAY TO START**. The next qualifying control input is retained at session time zero and starts the countdown. There is no focus gate.
