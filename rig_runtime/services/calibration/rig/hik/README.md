...
under `rig_runtime.adapters.hik`; Android presentation lives under
`rig_runtime.adapters.android`; orchestration and commands live under
`rig_runtime.workflows` and `rig_runtime.apps`. They reuse the repository's
...
```text
rig_runtime.adapters.hik.driver:create_camera_adapter
```
...
```powershell
python -m rig_runtime.apps.hik_rig_calibration --list-cameras --mvs-python-path C:\path\to\MvImport
```
...
```powershell
python -m rig_runtime.apps.hik_rig_calibration ... --headless --save
```
...
After ChArUco establishes the coarse phone orientation, calibration measures the
...
copied into frame-space metadata) for the measured panel-up vector in full-sensor
camera coordinates.
...
```powershell
python -m rig_runtime.apps.hik_stream calibration-output\hik_camera_calibration.json --gui
```
...
```python
from rig_runtime.apps.hik_stream import open_camera
