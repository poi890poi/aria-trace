...
`android/phone-target/iris-phone-target.apk`. Rig calibration resolves and
...
.\android\phone-target\build-phone-target.bat `
  -Output .\android\phone-target\iris-phone-target.apk
```

Without `-Output`, the build remains a local verification build at
`artifacts/android-phone-target/iris-phone-target.apk`. The calibrator installs
an APK only when the package is absent; an already installed compatible target
is launched directly. Set `IRIS_PHONE_TARGET_APK` or pass
`--phone-target-apk` to use another build explicitly. Standalone releases carry
the same binary at `phone-target/iris-phone-target.apk`.
