...
if (-not $OutputDirectory) {
    $OutputDirectory = Join-Path $ProjectRoot "artifacts\standalone-release\AriaTrace-Windows-x64"
}
...
$GitChanges = @(& git -C $ProjectRoot status --porcelain --untracked-files=no 2>$null)
$SourceTreeState = if ($GitChanges.Count -eq 0) { "clean" } else { "modified" }
...
$Manifest = @(
    ...
    "schema_version: '1.0'",
    "product: aria-trace-hik-calibration",
